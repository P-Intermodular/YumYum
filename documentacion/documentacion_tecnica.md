...
*   **Backend & Base de Datos:**
    *   **Supabase (PostgreSQL):** Elegido como BaaS. Provee base de datos relacional potente, autenticación delegada, control de acceso a nivel de fila (RLS), almacenamiento de archivos (Storage), notificaciones en tiempo real vía WebSockets (Realtime) y **Edge Functions** (Deno + TypeScript) para integraciones con servicios externos — sin necesidad de un backend Node propio.
    *   **Google Gemini (vía Edge Function):** Modelo conversacional del asistente IA. Se invoca con *function calling* desde la Edge Function `asistente-ia`, que orquesta la búsqueda de productos cercanos en la base de datos y compone la respuesta natural. La `GEMINI_API_KEY` vive como secret de Supabase y nunca se expone al cliente.
*   **Infraestructura:**
...
*   **`core/services/`:** Servicios transversales no ligados a ninguna feature concreta. Actualmente solo `IAService`, que invoca la Edge Function `asistente-ia` de Supabase (definida en `supabase/functions/asistente-ia/`) mediante `Supabase.instance.client.functions.invoke(...)`. La función habla con Gemini desde el servidor y la `GEMINI_API_KEY` vive como secret de Supabase.
...
**Servicios externos integrados como Edge Function:**
*   `IAService` (cliente Flutter) invoca la Edge Function `asistente-ia` (`supabase/functions/asistente-ia/index.ts`, Deno + TypeScript) mediante el SDK oficial: el cliente envía el historial multi-turno (`mensajes[]`) y la ubicación aproximada del usuario; el SDK adjunta automáticamente el JWT y la URL del proyecto. La función:
    1.  Decodifica el `uid` del JWT en local y carga el perfil del propio usuario (`nombre`, `ciudad`, `alergenos`, `bio`) respetando RLS, para personalizar el *system prompt* (la IA respeta los alérgenos declarados sin que se le pidan).
    2.  Llama a Google Gemini con *function calling* declarando la herramienta `buscar_productos_cercanos(consulta, categoria, tipo, max_precio)`. Si el modelo decide invocarla, la Edge Function ejecuta la RPC `obtener_productos_cercanos` con el JWT del usuario (respeta RLS y permisos columnares), filtra por criterios duros (categoria/tipo/precio) y devuelve los productos reales al modelo en un segundo turno con `responseSchema` que garantiza la forma del JSON final.
    3.  Aplica **fallback automático en cascada** entre seis modelos de Gemini (`gemini-2.5-flash-lite` por defecto, después `gemini-flash-lite-latest`, `gemini-3-flash-preview`, `gemini-2.0-flash-lite`, `gemini-flash-latest`, `gemini-2.5-flash`): si uno responde `429`/`500`/`503`, intenta el siguiente sin que el usuario note nada. El modelo preferido puede sobreescribirse con el secret `GEMINI_MODEL` sin redeploy.
    4.  Si todos los modelos fallan o la búsqueda devuelve 0 productos, **corta sin volver a llamar a la IA** y responde con un mensaje conversacional fijo (anti-alucinación + tolerancia a fallos), pintado en la burbuja del asistente como respuesta normal en vez de error.
*   La respuesta al cliente tiene la forma `{ respuesta: string, accion: 'buscar'|'publicar'|'info'|'ninguna', productos: ProductoSugerido[], prefilled_publicacion?: {...} }`. La `GEMINI_API_KEY` vive como **secret de Supabase** y jamás sale del servidor.
...
*   **Asistente IA conversacional desacoplado en una Edge Function:** El FAB `BotonIAGlobal` abre un bottom sheet multi-turno tipo chat (con sugerencias iniciales, burbujas usuario/asistente, loading dinámico y chips de producto navegables al detalle). El cliente envía el historial completo en cada turno; toda la lógica de negocio (perfil personalizado, *function calling* contra la RPC `obtener_productos_cercanos`, cascada de fallback entre modelos, anti-alucinación) vive en la Edge Function `asistente-ia`. La `GEMINI_API_KEY` nunca llega al cliente, los errores de Gemini se traducen a mensajes conversacionales y la UI nunca muestra detalles técnicos al usuario.
...
*   **Prefill del formulario de publicación desde el asistente IA pendiente:** La Edge Function ya devuelve `prefilled_publicacion` con los campos detectados (`titulo`, `descripcion`, `categoria`, `tipo`, `precio`), y el chip "Empezar a publicar" del chat enruta a `/publicar`, pero los datos no se inyectan en `PublicarProductoController`. Se necesita un provider transitorio (`borradorAsistenteProvider`) que el formulario lea al construirse para pre-rellenar los campos.
*   **Persistencia de conversación del asistente:** El historial del chat IA vive en el state de `_AsistenteSheetState` y se pierde al cerrar el bottom sheet (cada apertura empieza con conversación nueva). Para una UX más cercana a productos como ChatGPT habría que persistir en `SharedPreferences` o, si se quiere cross-device, en una tabla `conversaciones_ia` de Supabase.
...
7.  **Completar la orquestación del asistente IA (Prioridad Media):** Inyectar el `prefilled_publicacion` que ya devuelve la Edge Function dentro de `PublicarProductoController` mediante un provider transitorio, de modo que el botón "Empezar a publicar" del chat abra el formulario con título, categoría, tipo y precio ya rellenados. Como mejoras adicionales: añadir un botón "Ver todos en el feed" tras una búsqueda que vuelque los filtros al feed principal, persistir la conversación entre sesiones y declarar más herramientas a Gemini (`obtener_mis_publicaciones`, `obtener_favoritos`) para ampliar capacidades sin tocar el cliente.
...
9.  **Hardening menor pendiente (Prioridad Baja):** Cinco ajustes de bajo impacto visible pero útiles para producción: (a) alinear la policy `SELECT` de `imagenes_producto` con la visibilidad efectiva del producto (propietario, participantes de transacción o producto `disponible`); (b) añadir un `CHECK` constraint a `notificaciones.tipo` para acotar el dominio actual y prevenir tipos espurios; (c) completar el recálculo de `valoracion_media` con triggers `AFTER DELETE` y `AFTER UPDATE` sobre `valoraciones`, contemplando el caso de que `valorado_id` cambie en un UPDATE; (d) auditar los usos de `ref.keepAlive()` en los controladores de Riverpod y documentar los que se conserven; (e) terminar el hardening de Supabase activando "Leaked password protection" en Auth, valorando mover `cube` y `earthdistance` al schema `extensions`, y purgando los índices marcados como `unused` cuando exista tráfico real.
...
### 12.12. Asistente IA conversacional (FAB en Inicio)
*   **Actores implicados:** Usuario Autenticado, Cliente Flutter (`BotonIAGlobal` + `IAService`), Edge Function `asistente-ia` (Deno + TypeScript), RPC `obtener_productos_cercanos` de Supabase, API de Google Gemini.
*   **Precondiciones:** El usuario debe estar autenticado (la Edge Function se despliega con `verify_jwt: true`), `GEMINI_API_KEY` debe estar configurada como secret de Supabase y, para que la búsqueda devuelva resultados reales, el usuario debe haber concedido permiso de ubicación.
*   **Pasos:**
    1.  Desde Inicio, el usuario pulsa el FAB `BotonIAGlobal`. Se abre un `DraggableScrollableSheet` con bienvenida + chips de sugerencias (`"¿Qué hay para comer cerca?"`, `"Quiero algo dulce"`, `"Planifica mi comida de hoy"`, `"Ayúdame a publicar un plato"`).
    2.  Al enviar (texto libre o pulsando una sugerencia), `_AsistenteSheetState` añade el turno del usuario al historial, dispara `IAService.enviarHistorial(mensajes, lat, lon)` y muestra una burbuja de loading con mensajes rotatorios (*"Pensando…"*, *"Buscando platos cerca de ti…"*…).
    3.  La Edge Function valida el JWT, decodifica el `uid` en local del propio JWT (sin un round trip a `auth.getUser`), carga el perfil (`nombre`, `ciudad`, `alergenos`, `bio`) respetando RLS y construye un `system prompt` personalizado que incluye, entre otras reglas, el respeto obligatorio de los alérgenos declarados y la prohibición de inventar productos o distancias.
    4.  Reconstruye el historial multi-turno en el formato de Gemini (`role: 'user'|'model'`) y lo manda al primer modelo de la cascada (`gemini-2.5-flash-lite` por defecto, sobreescribible con el secret `GEMINI_MODEL`) declarando la herramienta `buscar_productos_cercanos`.
    5.  **Si Gemini invoca la herramienta** (caso búsqueda), la Edge Function ejecuta la RPC `obtener_productos_cercanos` con la ubicación del usuario, radio de 25 km y un máximo de 30 candidatos, filtra los resultados por los argumentos elegidos por el modelo (categoría, tipo, precio máximo) y se queda con los 8 mejores. Pasa ese listado a Gemini en un segundo turno con `responseSchema` para garantizar el JSON final.
    6.  **Cortocircuito anti-alucinación:** si la RPC devuelve 0 productos después del filtrado, la Edge Function NO vuelve a llamar al modelo; responde directamente con `{ respuesta: "No he encontrado platos cercanos…", accion: "buscar", productos: [] }`. Así se evita que el modelo invente distancias o platos cuando no tiene datos reales que usar.
    7.  **Si Gemini no invoca la herramienta** (saludo, ayuda, intención de publicar), la Edge Function pide una segunda llamada estructurada para identificar `accion` (`'publicar'` / `'info'` / `'ninguna'`) y, si procede, devolver `prefilled_publicacion` con los campos detectados.
    8.  **Fallback en cascada:** si cualquier llamada a Gemini devuelve `429` (cuota agotada), `500` o `503` (servidor saturado), la Edge Function reintenta automáticamente con el siguiente modelo de la lista prioritaria hasta que uno responde. Los `400`/`401`/`404` no entran en la cascada (es error nuestro o de configuración).
    9.  El cliente recibe `{ respuesta, accion, productos[], prefilled_publicacion? }` y pinta una nueva burbuja del asistente. Si hay productos, se renderizan como chips clickables que navegan al detalle al pulsarlos. Si `accion === 'publicar'` o `prefilled_publicacion` no es nulo, aparece el botón "Empezar a publicar" que cierra el sheet y enruta a `/publicar`. Si la lista de productos quedó vacía tras buscar, la burbuja incluye un mensaje contextual con CTA a publicar.
    10. Solo el último turno del asistente expone botones de acción; los anteriores quedan estáticos pero los chips de producto siguen siendo navegables. El botón ↻ de la cabecera reinicia la conversación.
*   **Excepciones / Casos de error:**
    *   Si todos los modelos de la cascada fallan (típicamente todos con `429`), la Edge Function responde con `200` y un mensaje conversacional: *"Hoy ya he ayudado a mucha gente y se me ha agotado el cupo del día. Vuelve a probar mañana"* (o variante para `503`). El usuario lo ve como una burbuja normal del asistente, no como error.
    *   Si Gemini devuelve un error no recuperable (`400` / `404` por modelo no compatible), la Edge Function responde `500` con un mensaje legible (*"Algo no ha ido bien por mi parte. Vuelve a intentarlo en un momento"*) y el cliente lo pinta como banner inline encima del input sin bloquear la conversación.
    *   Si la app no consigue resolver la ubicación (permiso denegado o GPS apagado) y el usuario pide buscar productos, la Edge Function detecta que faltan `latitud`/`longitud` y responde con un aviso explicando que necesita ubicación.
    *   Si el usuario no está autenticado, la Edge Function rechaza la petición por falta de JWT (`401`) y el cliente lo refleja en el banner de error inline.
    *   Pérdida de red en el cliente: `IAService` traduce la excepción a *"Sin conexión con el asistente. Comprueba tu Internet y vuelve a intentarlo"*; el historial previo se conserva intacto para reintentar.
