...
*   `IAService` invoca la Edge Function `asistente-ia` de Supabase (`supabase/functions/asistente-ia/`) mediante el SDK oficial: el cliente adjunta automáticamente el JWT del usuario y la URL del proyecto. La función habla con la API de Google Gemini (`gemini-2.5-flash`) usando la `GEMINI_API_KEY` configurada como **secret de Supabase**, así que la clave nunca sale del servidor. Devuelve JSON estructurado (`accion`, `resumen` y campos opcionales) con `responseSchema` para garantizar la forma de la respuesta.
...
*   **FAB de asistente IA aislado:** El componente `BotonIAGlobal` vive en `core/widgets/ui/` pero se monta solamente en la pantalla de Inicio (no en el shell), por lo que no compite con el FAB '+' del bottom nav. `IAService` invoca la Edge Function `asistente-ia` de Supabase mediante el SDK oficial (`functions.invoke`), de forma que toda la integración con Gemini queda en el servidor, la `GEMINI_API_KEY` no se expone al cliente y los errores se aíslan en su propio SnackBar.
...
*   **Orquestación del intent del asistente IA pendiente:** La Edge Function `asistente-ia` ya devuelve un JSON estructurado (`accion`, `resumen`, campos opcionales), pero el cliente todavía no enruta a publicar / filtra el feed según `accion`; el `_FormularioIa` solo imprime la respuesta con `debugPrint`.
...
7.  **Cablear el intent devuelto por el asistente IA (Prioridad Media):** Consumir `accion` y los campos extraídos (`nombre`, `categoria`, `tipo`, `precio`, `descripcion`) desde `_FormularioIa` para abrir prefilled la pantalla de publicación o aplicar filtros al feed. Añadir un timeout explícito y un feature-flag para esconder el FAB si la Edge Function devuelve `503/502`.
...
*   **Actores implicados:** Usuario Autenticado, Sistema, Edge Function `asistente-ia`, API de Google Gemini.
*   **Precondiciones:** El usuario debe estar autenticado (la Edge Function se despliega con `verify_jwt: true`) y `GEMINI_API_KEY` debe estar configurada como secret del proyecto Supabase.
*   **Pasos:**
    1.  Desde la pantalla de Inicio, el usuario pulsa el FAB `BotonIAGlobal` y se abre un bottom sheet con un campo de texto.
    2.  Al enviar, `IAService.procesarTexto` invoca la Edge Function `asistente-ia` vía `Supabase.instance.client.functions.invoke(...)`. El SDK adjunta automáticamente el JWT del usuario, la URL del proyecto y la `anon key`.
    3.  La Edge Function valida el payload, construye el prompt y llama a `gemini-2.5-flash` con `responseMimeType: application/json` y un `responseSchema` que garantiza la forma del resultado.
    4.  Devuelve un JSON con `accion` (`'publicar'` | `'buscar'` | `'desconocido'`), un `resumen` siempre presente y campos opcionales (`nombre`, `categoria`, `tipo`, `precio`, `descripcion`).
    5.  Actualmente la respuesta solo se imprime con `debugPrint`; el cableado de la navegación al intent (publicar/filtrar feed) está pendiente.
*   **Excepciones / Casos de error:**
    *   Si la Edge Function devuelve `{ "error": "..." }`, `IAService` lanza una excepción con ese mensaje y la UI lo muestra en un SnackBar.
    *   Si Gemini falla (`502`) o falta la API key (`500`), el cliente recibe un mensaje genérico de "asistente no disponible" sin afectar al resto de la app.
    *   Si el usuario no está autenticado, la Edge Function rechaza la petición por falta de JWT.
