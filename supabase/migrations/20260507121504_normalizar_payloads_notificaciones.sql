-- Normaliza los payloads `datos` de las notificaciones y corrige tildes en
-- los textos visibles. Cada notificación de solicitud lleva ahora
-- `solicitud_id` + `producto_id` + `conversacion_id`; las de transacción
-- añaden `transaccion_id`. Esto da simetría al payload y permite que el
-- frontend enrute por `tipo` con un fallback robusto.
--
-- No cambian firmas ni grants; solo se reescriben los cuerpos de las RPCs
-- vigentes que insertan en `public.notificaciones`.

-- ============================================================
-- crear_solicitud_oferta: añade conversacion_id al payload.
-- ============================================================
...
-- ============================================================
-- aceptar_solicitud_oferta: añade producto_id al payload de "aceptada" y
-- conversacion_id al de "auto_denegada" (vía LEFT JOIN con conversaciones).
-- ============================================================
...
begin
  ...
  -- LEFT JOIN con conversaciones para incluir conversacion_id en el payload.
  with denegadas as (
  ...
  insert into public.notificaciones (usuario_id, tipo, titulo, contenido, datos)
  select d.solicitante_id, 'solicitud_oferta_auto_denegada', 'Solicitud no disponible',
    'Una solicitud relacionada ya no está disponible porque otra fue aceptada.',
    jsonb_build_object(
      'solicitud_id', d.id,
      'producto_id', d.producto_id,
      'producto_ofrecido_id', d.producto_ofrecido_id,
...
-- ============================================================
-- denegar_solicitud_oferta: añade conversacion_id al payload.
-- ============================================================
...
-- ============================================================
-- cancelar_solicitud_oferta: añade conversacion_id al payload.
-- ============================================================
...
-- ============================================================
-- cancelar_transaccion: añade solicitud_id y conversacion_id, corrige tildes.
-- ============================================================
...
begin
  ...
  select id into v_conversacion_id
  from public.conversaciones
  where solicitud_id = v_transaccion.solicitud_id;
  ...
  values (
    ...
    'Una transacción ha sido cancelada por la otra parte.',
    jsonb_build_object(
